...
export default function Result({ rule, matchedFacts }) {
    const hasMatch = rule !== null;
                    ...
                    <div className={`rounded-2xl shadow-xl overflow-hidden border ${hasMatch ? 'border-red-200' : 'border-green-200'}`}>
                        ...
                        <div className={`px-6 py-8 sm:p-10 text-center ${hasMatch ? 'bg-gradient-to-b from-red-600 to-red-700' : 'bg-gradient-to-b from-green-500 to-green-600'}`}>
                            ...
                            <h2 className="text-3xl font-extrabold text-white mb-2">
                                {hasMatch ? 'Violation Detected' : 'No Violation Detected'}
                            </h2>
                            <p className="text-white/80 text-lg">
                                {hasMatch ? rule.goal_name : 'The provided facts do not map to any known fouls.'}
                            </p>
                        </div>
                        ...
                        <div className="bg-white px-6 py-8 sm:p-10">
                            {hasMatch && (
                                <div className="mb-10">
                                    <h3 className="text-sm font-bold tracking-widest text-red-600 uppercase mb-3">System Conclusion</h3>
                                    <div className="p-5 bg-red-50 rounded-xl border border-red-100 text-red-900 md:text-lg leading-relaxed shadow-sm">
                                        {rule.output_description}
                                    </div>
                                </div>
                            )}
                            ...
                            <div>
                                ...
                                {matchedFacts && matchedFacts.length > 0 ? (
